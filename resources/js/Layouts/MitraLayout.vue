...
const navigationGroups = [
  {
    title: 'MAIN MENU',
    items: [
      { name: 'Dashboard', href: route('mitra.dashboard'), icon: LayoutDashboard, current: route().current('mitra.dashboard') },
    ]
  },
  {
    title: 'CONTENT',
  ...
  },
  {
    title: 'PROFILE',
    items: [
      { name: 'Profile Mitra', href: route('mitra.profile'), icon: User, current: route().current('mitra.profile') },
    ]
  }
];
    ...
    >
      <div>
        ...
        <div :class="[isSidebarCollapsed ? 'lg:px-2 lg:justify-center' : 'px-6 justify-between', 'flex items-center h-20 border-b border-emerald-950/20']">
          <Link :href="route('mitra.dashboard')" class="flex items-center gap-3">
            ...
            <div v-if="!isSidebarCollapsed" class="transition-opacity duration-300">
              <h1 class="text-xs font-bold tracking-tight text-white uppercase">Mitra Panel</h1>
            </div>
          </Link>
          <button 
            @click="isSidebarOpen = false" 
