...
const handleEdit = (item) => {
  router.get(route('mitra.articles.edit', item.id));
};

const toggleEditorPick = (item) => {
  router.patch(route('mitra.articles.toggle-editor-pick', item.id), {}, {
    preserveScroll: true
  });
};
...
const handleDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    router.delete(route('mitra.articles.destroy', item.id));
  }
};
...
const handleBulkDelete = async () => {
  ...
  if (result.isConfirmed) {
    router.delete(route('mitra.articles.bulk-destroy'), {
    ...
    <div class="space-y-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <Link 
            :href="route('mitra.articles.create')"
